...
-- Treesitter setup (nvim-treesitter v0.9+ API)
-- Enable built-in treesitter highlighting for all filetypes
vim.api.nvim_create_autocmd('FileType', {
    callback = function() pcall(vim.treesitter.start) end,
})

-- Auto-install parsers on startup (skips already-installed ones)
vim.api.nvim_create_autocmd('VimEnter', {
    once = true,
    callback = function()
        require('nvim-treesitter').install({ "lua", "javascript", "typescript", "python", "go", "rust", "html", "css", "json", "markdown", "terraform", "hcl" })
    end,
})

-- Textobject move keymaps (nvim-treesitter-textobjects new API)
